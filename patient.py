...
def register_patient(first_name, last_name, dob, gender, contact_number, email, password):
    connection = connect_to_database()
    if connection:
        try:
            cursor = connection.cursor()
            query = """
            INSERT INTO patients (first_name, last_name, dob, gender, contact_number, email, password)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            """
            values = (first_name, last_name, dob, gender, contact_number, email, password)
...
def submit_form():
    ...
    if not all([first_name, last_name, dob, gender, contact_number, email, password]):
        messagebox.showwarning("Input Error", "All fields are required")
        return

    register_patient(first_name, last_name, dob, gender, contact_number, email, password)
...
tk.Button(root, text="Register", command=submit_form).grid(row=7, column=1, columnspan=2)
